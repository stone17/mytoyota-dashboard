document.addEventListener('DOMContentLoaded', () => {
    ...
    async function showTripOnMap(trip, rowIndex) {
        ...
        mapPanelTitle.innerHTML = `Trip on ${window.safeLocaleDateString(trip.start_timestamp, undefined, { timeZone: window.appConfig?.timezone || 'UTC' })} 
    ...
    function renderTable(trips) {
        ...
        const formatTimestamp = (ts) => !ts ? 'N/A' : `${window.safeLocaleDateString(ts, undefined, { timeZone: window.appConfig?.timezone || 'UTC' })}<br><span class="unit">${window.safeLocaleTimeString(ts, undefined, { timeZone: window.appConfig?.timezone || 'UTC' })}</span>`;
